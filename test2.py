...
param_grid = {
    ...
    'model__dropout_rate': [0.2, 0.3, 0.4, 0.5],  # Increased range of dropout rates
    'optimizer': ['adam', 'rmsprop'],
    'batch_size': [16, 32, 64],  # Include smaller batch sizes
    'epochs': [50, 100, 200],  # Include larger number of epochs
    'fit__validation_split': [0.1, 0.2]  # Add validation split parameter
}

# Cross-validation and hyperparameter tuning
kfold = KFold(n_splits=3, shuffle=True, random_state=42)
random_search = RandomizedSearchCV(estimator=model, param_distributions=param_grid, n_iter=20, cv=kfold, random_state=42, verbose=1)
...
plt.plot(history.get('val_loss', []), label='Validation Loss')  # Use get to avoid KeyError
